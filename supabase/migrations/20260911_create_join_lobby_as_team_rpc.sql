-- ==============================================================================
-- MIGRATION: Create secure RPC join_lobby_as_team
-- Description:
-- 1. Creates SECURITY DEFINER function public.join_lobby_as_team
-- 2. Validates caller authentication, captain ownership, and approved team roster
-- 3. Validates lobby status, remaining slot capacity, and participant uniqueness
-- 4. Rejects banned players and non-existent profiles
-- 5. Atomically inserts selected team members into public.lobby_participants with team_id
-- 6. Configures search_path = '' and restricts EXECUTE to authenticated users
-- ==============================================================================
...
BEGIN
  ...
  -- 10. Lock & verify target lobby status and slot capacity
  SELECT status, slot_count
  INTO v_lobby_status, v_lobby_slot_count
  FROM public.lobbies
  ...
  -- 11. Check current participants count and remaining capacity
  SELECT pg_catalog.count(*)
  ...
  -- 12. Check if any selected member is already in this lobby
  SELECT pg_catalog.count(*)
  ...
  -- 13. Atomically insert all selected members with team_id
